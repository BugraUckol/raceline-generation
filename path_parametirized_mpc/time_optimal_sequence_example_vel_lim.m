...
lw = 2.0; % Wheelbase
steering_lim = 0.5;
c = 0.0; % Viscous friction of the longitudinal dynamics
...
steer_rate = (U(2,1:end-1) - U(2,2:end)); %./ (X(2,1:end-2) - X(2,2:end-1));

% ---- objective          ---------
% opti.minimize(t(end)); % minimize time
opti.minimize(0.0*dot(U(2,:), U(2,:)) + 1.0 * t(end) + 10 * dot(steer_rate, steer_rate)); % minimize steering
...
for k=1:N % loop over control intervals
   ...
   % Curvature related speed limits
   if abs(kappa) > 0.01
    curve_lim = sqrt(max_lat_acc / abs(tan(U(2,k)) / lw));
    opti.subject_to(X(2,k+1) <= curve_lim);
   end
end
...
guess_speed(guess_speed < min_speed) = min_speed;
guess_speed(guess_speed > max_speed) = max_speed;

% ---- Initial guess
% Problem is symmetric when the pendulum is pointing down or up
%   adding a random initial guess helps it to solve
% opti.set_initial(X(1,:), 1:N+1);
opti.set_initial(X(2,:), [v0, guess_speed]);
...
traj_x = x_y_yaw_curv_s_lx_ly_rx_ry_dl_dr(1:N+1,1) + lat_err_arr' ...
    .* -sin(x_y_yaw_curv_s_lx_ly_rx_ry_dl_dr(1:N+1,3));
traj_y = x_y_yaw_curv_s_lx_ly_rx_ry_dl_dr(1:N+1,2) + lat_err_arr' ...
    .* cos(x_y_yaw_curv_s_lx_ly_rx_ry_dl_dr(1:N+1,3));
...
plot(distance_arr, x_y_yaw_curv_s_lx_ly_rx_ry_dl_dr(1:N+1, 10), '-.', 'LineWidth', 2)
plot(distance_arr, x_y_yaw_curv_s_lx_ly_rx_ry_dl_dr(1:N+1, 11), '-.', 'LineWidth', 2)
