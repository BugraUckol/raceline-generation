...
% System dynamics
f = @(tt,en,eb,e_phi,e_the,e_psi,u,v,w,p,q,r,Fz,Mx,My,Mz,kappa,tau,gx_p, gy_p, gz_p) [
    % dt / ds
...
% opti.subject_to(en >= -0.5);
% opti.subject_to(eb >= -0.5);
opti.subject_to(en.^2 + eb.^2 <= tube_rad^2);
...
plotCircle3D([path.x_arr(ei), path.y_arr(ei), path.z_arr(ei)], path.tt_arr(:,ei)', tube_rad, '-k', 0.05)
end
edges_ei = edges_e_list(:,:,1);
% plot3(edges_ei(1,:), edges_ei(2,:), edges_ei(3,:), 'g', 'LineWidth', 4), hold on
plotCircle3D([path.x_arr(1), path.y_arr(1), path.z_arr(1)], path.tt_arr(:,1)', tube_rad, '-g', 5)
edges_ei = edges_e_list(:,:,end-1);
% plot3(edges_ei(1,:), edges_ei(2,:), edges_ei(3,:), 'r', 'LineWidth', 4), hold on
plotCircle3D([path.x_arr(end-1), path.y_arr(end-1), path.z_arr(end-1)], path.tt_arr(:,ei)', tube_rad, '-r', 5)
...
% plot3(path.x_arr, path.y_arr, path.z_arr, 'LineWidth', 2), hold on
% plot3(x_arr, y_arr, z_arr, 'LineWidth', 4, 'color', 'black'), hold on
clf
x = x_arr;
y = y_arr;
z = z_arr;
...
edges_ei = edges_e_list(:,:,end-1);
% plot3(edges_ei(1,:), edges_ei(2,:), edges_ei(3,:), 'r', 'LineWidth', 4), hold on
plotCircle3D([path.x_arr(end), path.y_arr(end), path.z_arr(end)], path.tt_arr(:,ei)', tube_rad, '-r', 5)
...
for k=1:5:N-1
    ...
    % vv = [u_arr(k); v_arr(k); w_arr(k)];
    % ve = dcm_b_e * vv / abs(vv);
    % vx = [p, p + ve];
    % plot3(vx(1,:), vx(2,:), vx(3,:), 'LineWidth', 2, 'Color', 'm');
...
%     '$$\hat{x_{\mathcal{B}}}$$', '$$\hat{y_{\mathcal{B}}}$$', ...
%     '$$\hat{z_{\mathcal{B}}}$$'}, 'interpreter', 'latex', 'Location','northeast')
legend({'Optimal Trajectory', 'Start', 'Finish', ...
    '$$\hat{z_{\mathcal{B}}}$$', '$$\vec V$$'}, 'interpreter', 'latex', 'Location','northeast')
