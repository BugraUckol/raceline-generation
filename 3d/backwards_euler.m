...
for k=2:N+1 % loop over control intervals
    if k <= N
        ...
        tau = path.tau_arr(1);
        kappa1 = path.kap1_arr(1);
        kappa2 = path.kap2_arr(1);
        ds = path.s_arr(1 + 1) ...
            - path.s_arr(1);
    
        earth_axes_in_earth = CE2B([path.roll_arr(1), path.pitch_arr(1), path.yaw_arr(1)]);
        gravity_vec = g * -earth_axes_in_earth(:,3);
    end

    % 1st Order Explicit Euler's Integration
    k1 = f(X(1,k), X(2,k), X(3,k), X(4,k), X(5,k), X(6,k),...
        X(7,k), X(8,k), X(9,k), X(10,k), X(11,k), X(12,k), U(1,k-1), U(2,k-1), U(3,k-1), U(4,k-1), ...
    ...
    opti.subject_to(FM2w * U(:,k-1) <= [1,1,1,1]');
    opti.subject_to(FM2w * U(:,k-1) >= [0,0,0,0]');
...
y = y_arr;
z = z_arr;
patch([x nan],[y nan],[z nan],[time_arr nan], 'edgecolor', 'interp','LineWidth',5);
hcb = colorbar;colormap(jet);grid minor;
colorTitleHandle = get(hcb,'Title');
titleString = 'Time';
set(colorTitleHandle ,{'String','Rotation','Position'},{ titleString,90,[80 250]});
